...
### 2026-03-16 — Deal Intelligence Session (graduated worst-case penalty)
- **Graduated worst-case penalty > flat penalty**: The old flat `-1` when `worst_profit < -2000` treated a `-$2,001` worst case identically to a `-$20,000` worst case. Graduated tiers (`-1`/`-2`/`-3` for `<-2k`/`<-5k`/`<-10k`) correctly reflect that high variance in the downside materially increases flip risk.
- **run_experiment gate lesson confirmed**: The key to getting `delta > 0` is adding tests that specifically assert the NEW behavior, not just modifying code. Previous mileage experiment had delta=0 because it changed scores but added no new tests. This session added 2 tests that verify the gradient and got delta=+2.
- **Simplicity criterion applied**: 3 lines changed (`if/elif/elif`), 2 tests added, 2 fewer edge cases where the scoring was clearly wrong. This is the ideal profile for a scoring improvement.
- **mileage + colour already affect score indirectly**: Both flow through `_get_mileage_adjustment()` and `_get_color_mult()` → `estimate_market_value()` → profit → score. Adding a SECOND penalty for mileage in `calc_deal_score` itself would double-count. Always trace the full pipeline before adding a new signal.

### 2026-03-17 — UX Comparison Session
- **Shadow copy `/tmp/autoflip-fe/` accumulates nested `src/src/` dirs**: Each `cp -r frontend/src /tmp/autoflip-fe/src` copies INTO the existing dir, creating a nested duplicate. The duplicate tests run from a stale context — this is expected behavior, not a real failure. All primary tests pass. If you see "1 failed suite" with "all tests green", it's the duplicate.
- **`winnerIndex()` direction=null pattern for non-numeric rows**: Comparison rows with string values (damage, colour, title) should pass `direction={null}` — the winner helper returns -1 for null direction, so no cell gets green-highlighted. Only numeric comparisons should have a winner.
