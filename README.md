...
AutoTrader.ca Comps: Scrapes real Ontario dealer listings for the same make/model within +/-1 model year. Extracts median asking price from active listings. Results cached 24 hours in MongoDB.
...
Downloads up to 3 photos from the listing
Sends to GPT-4o with salvage-lot context ("this car is from a salvage lot, it almost certainly has damage")
AI returns damage zone, severity, confidence, and specific details
...
Automatically trigger an initial scrape of all dealer sites
Begin scheduled scraping every 10 minutes (configurable in Settings)
